...
import { queryInDatabase } from "../../utils/userFunctions";
...
async function userRegister(req: any, res: any) {
  ...
  */
    ...
    try {
      ...
      const createUserPhoneNoResult: boolean = await queryInDatabase(
        queryCreatePhoneNo,
        userPhoneNo,
        pool
      );
      
      if (createUserPhoneNoResult) {
        try {
          ...
          const createUserResult: boolean = await queryInDatabase(
            queryUser,
            user,
            pool
          );
        ...
        try {
          ...
          const deletePhoneNoResult: boolean = await queryInDatabase(
            queryDeletePhoneNo,
            params,
            pool
          );

          if (!deletePhoneNoResult) {
            res.json({
